...
void StaticVBO_inFBO(int view, int thread, int id)
{
	...
	glUniform1i(glGetUniformLocation(ShaderProjID, "thread"), thread);
...
void drawEntire_forFPGA(int thread)
{
	...
	for (int i = 0; i<100; i++)
	{
		...
		{
			glEnable(GL_DEPTH_TEST);
			StaticVBO_inFBO(200 + i, thread, 0); StaticVBO_inFBO(0 + i, thread, 1);	StaticVBO_inFBO(100 + i, thread, 2);
		}
